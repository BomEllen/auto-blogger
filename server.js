...
[절대 지켜야 할 규칙]
- 마크다운 문법 완전 금지: **굵게**, *기울임*, ##헤더, - 목록 기호 등 일체 사용 금지. 순수 텍스트만 작성
- 단락 사이는 빈 줄(줄바꿈 2번)로 구분해 가독성을 높일 것
- 소제목은 앞에 기호를 붙이거나 꺾쇠를 사용하지 말고 텍스트 그대로 한 줄에 쓸 것 (예: ▶ 위치 안내 → 위치 안내)
- 리스트가 필요하면 기호 없이 줄바꿈으로만 나열할 것
